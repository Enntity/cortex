...
export const DEFAULT_CONFIG = {
    ...
    contextCacheTTL: 300, // 5 minutes - for topic-specific context
    bootstrapCacheTTL: 600, // 10 minutes - for identity context (CORE, CORE_EXTENSION, relational)
...
/**
 * Check if a merge operation would cause unacceptable semantic drift.
 * Uses the "half-drift rule": merged content can drift from the NEW memory
 * by at most half the original gap between M and S.
 * 
 * This prevents "mega-memories" by detecting when an LLM expanded rather
 * than consolidated during a merge operation.
 * 
 ...
 * @returns {{valid: boolean, mergedToM: number, mergedToS: number, originalSim: number, minSimToM: number}}
 */
export function checkMergeDrift(mVector, sVector, mergedVector) {
    ...
    // Half-drift rule: M' can drift from M by at most half the original gap
    // minSimToM = (1 + originalSim) / 2
    // If originalSim = 0.8, then minSimToM = 0.9 (only 0.1 drift allowed)
    // If originalSim = 0.75, then minSimToM = 0.875 (only 0.125 drift allowed)
    const minSimToM = (1 + originalSim) / 2;
    
    // Merge is valid if:
    // 1. M' stays close to M (preserves the new information)
    // 2. M' doesn't drift further from S than M was originally
    const valid = mergedToM >= minSimToM && mergedToS >= originalSim;
