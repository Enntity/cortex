 ...
 * @property {{score: number, driftNotes: string}|null} authenticityAssessment - Eidos self-assessment
 * @property {{lengthFeel: string, modelInfluence: string|null, toneMismatch: string|null, correction: string|null}|null} voiceCheck - Eidos voice consistency check
 */
...
export const DEFAULT_CONFIG = {
    ...
    // Each run processes maxMemoriesPerRun memories in batches of 50 with 20% overlap.
    // The system self-heals by working through the full backlog newest-first,
    // consolidating ~500 memories per run (~12 LLM calls) on a 4-hour cooldown.
    deepSynthesis: {
        runOnSessionEnd: true,                     // Run consolidation when session expires
        maxMemoriesPerRun: 500,                    // Memories per run (processed in batches of 50)
        daysToLookBack: 0,                         // 0 = no date limit — process full backlog newest-first
        cooldownHours: 4                           // Hours between runs per entity (6 runs/day max)
    },
    // Note: Synthesis models are now defined in the pathways themselves:
    // - sys_continuity_turn_synthesis: oai-gpt5-mini
    // - sys_continuity_deep_analysis: oai-gpt41
    // - sys_continuity_compass_synthesis: oai-gpt41-mini

    // Eidos: introspective monitoring layer
    // Observes entity behavior and feeds self-awareness into the Internal Compass
    eidos: {
        enabled: true,
        soulReport: {
            turnInterval: 100,
