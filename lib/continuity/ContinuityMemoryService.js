        ...
        setImmediate(async () => {
            try {
                ...
                if (DEFAULT_CONFIG.eidos?.enabled !== false) {
                    try {
                        // 1. Extract authenticity score from LLM-assessed synthesis result
                        const authenticityScore = result.authenticityAssessment?.score ?? null;

                        // 2. Update resonance metrics from synthesis result
                        ...
                        // 3. Update rolling authenticity scores (only if LLM provided one)
                        ...
                        // 6. Format for compass (include drift notes and voice check if available)
                        ...
                        // 7. Check for Soul Report trigger
                ...
                // Update Internal Compass to maintain "what we're doing now" context
                // This runs after every turn synthesis so the compass always reflects current activity
                const shouldSynthesizeCompass = DEFAULT_CONFIG.internalCompass?.synthesizeEveryTurn !== false;

                if (shouldSynthesizeCompass) {
                    // For incremental updates, only pass recent turns - existing compass has the history
                    // This keeps input minimal while the compass carries forward synthesized context
                    ...
                    if (recentBuffer.length >= minTurns) {
                        ...
                        // Log compass synthesis in continuity mode
    ...
    async triggerPulseSynthesis(entityId, context = {}) {
        ...
        setImmediate(async () => {
            try {
                ...
                // Eidos introspection during pulse
                let eidosMetricsStr = '';
                if (DEFAULT_CONFIG.eidos?.enabled !== false) {
                    try {
                        const authenticityScore = result.authenticityAssessment?.score ?? null;
                        const existingMetrics = await this.hotMemory.getEidosMetrics(
                            entityId,
                            ContinuityMemoryService.PULSE_USER_ID
                        );
                        const resonanceMetrics = this.resonanceTracker.computeMetrics(
                            result, existingMetrics?.resonanceMetrics
                        );
                        ...
                        const turnCount = await this.hotMemory.incrementEidosTurnCount(
                            entityId,
                            ContinuityMemoryService.PULSE_USER_ID
                        );

                        await this.hotMemory.updateEidosMetrics(
                            entityId,
                            ContinuityMemoryService.PULSE_USER_ID,
                            { authenticityScores: scores, resonanceMetrics, turnCount }
                        );
                        ...
                        logger.warn(`Pulse Eidos failed (non-fatal): ${eidosError.message}`);
                    }
                }

                // Update entity-level Internal Compass
                const recentBuffer = await this.hotMemory.getEpisodicStream(
                    entityId,
                    ContinuityMemoryService.PULSE_USER_ID,
                    DEFAULT_CONFIG.internalCompass?.recentTurnsForUpdate || 8
                );

                if (recentBuffer.length >= (DEFAULT_CONFIG.internalCompass?.minTurnsForSynthesis || 2)) {
                    await this.synthesizer.synthesizeInternalCompass(
                        entityId,
                        null, // Entity-level compass
                        recentBuffer,
                        { aiName: context.aiName || 'Entity', eidosMetrics: eidosMetricsStr }
                    );
                }
