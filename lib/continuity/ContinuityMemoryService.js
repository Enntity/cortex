...
// Memory backend selection: 'azure' or 'mongo'
// Set via CONTINUITY_MEMORY_BACKEND env var or defaults based on available config
function selectMemoryBackend() {
    const explicit = process.env.CONTINUITY_MEMORY_BACKEND?.toLowerCase();
    ...
    // Auto-detect: prefer Mongo if MONGO_URI is set, otherwise Azure
...
export class ContinuityMemoryService {
    ...
    constructor(options = {}) {
        ...
        // Select cold memory backend based on config/environment
        const backend = options.backend || selectMemoryBackend();
        ...
        if (backend === 'mongo') {
            ...
            logger.info('Continuity memory using Azure AI Search backend');
        }
    ...
    async _getBootstrapContext(entityId, userId, relationalLimit = 5, minImportance = 6) {
        ...
        try {
            // Cache miss - fetch from Azure (3 parallel calls)
            ...
            logger.debug(`Bootstrap cache miss: fetched ${coreMemories.length} core, ${relationalBase.length} relational from Azure`);
