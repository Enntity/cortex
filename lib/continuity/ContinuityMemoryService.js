 ...
 * - sys_entity_runtime.js / sys_entity_executor.js: Tool access for explicit memory queries
 */
...
export class ContinuityMemoryService {
    ...
    constructor(options = {}) {
        ...
        // Deep synthesis cooldown — per-entity timestamp of last run
        // In-memory Map is sufficient: survives within process lifetime,
        // and one fresh run after restart is acceptable.
        this.lastDeepSynthesisTime = new Map();
    ...
    async getContextWindow({ entityId, userId, query, options = {} }) {
        ...
        try {
            // ==================== PARALLEL FETCH: Hot Memory + Bootstrap ====================
            // All independent data sources fetched concurrently

            const [episodicStream, expressionState, activeCache, { coreMemories, relationalBase }] = await Promise.all([
                this.hotMemory.getEpisodicStream(entityId, userId, episodicLimit),
                this.hotMemory.getExpressionState(entityId, userId),
                this.hotMemory.getActiveContext(entityId, userId),
                this._getBootstrapContext(entityId, userId, bootstrapRelationalLimit, bootstrapMinImportance),
            ]);
            ...
            // Internal compass can be fetched in parallel with topic search
            let internalCompass = null;

            if (needsFreshTopicSearch && this.coldMemory.isConfigured()) {
                // Run topic search and compass fetch in parallel
                const [topicResult, compassResult] = await Promise.all([
                    this.coldMemory.searchSemantic(entityId, userId, queryStr, topicMemoryLimit),
                    this.synthesizer.getInternalCompass(entityId, userId),
                ]);
                topicMemories = topicResult;
                internalCompass = compassResult;

                // Graph expansion (depends on topic results)
                ...
                // Reuse cached topic memories + fetch compass in parallel
                const cachedIds = [
                    ...(activeCache.currentRelationalAnchors || []),
                    ...(activeCache.activeResonanceArtifacts || [])
                ];

                const [cachedMemories, compassResult] = await Promise.all([
                    cachedIds.length > 0 ? this.coldMemory.getByIds(cachedIds) : Promise.resolve([]),
                    this.synthesizer.getInternalCompass(entityId, userId),
                ]);
                topicMemories = cachedMemories;
                internalCompass = compassResult;
            } else {
                // No cold memory — just fetch compass
                internalCompass = await this.synthesizer.getInternalCompass(entityId, userId);
            }
            ...
            if (needsFreshTopicSearch && this.coldMemory.isConfigured()) {
                // Generate narrative summary and cache in background — don't block the response
                this.contextBuilder.generateNarrativeSummary(
                    entityId, userId, [...relevantMemories, ...expandedMemories], queryStr
                ).then(narrativeContext => {
                    return this.hotMemory.setActiveContext(entityId, userId, {
                ...
                }).catch(err => logger.warn(`Background narrative cache update failed: ${err.message}`));
            }
     ...
     * @private
     */
    async _processEidosAndCompass(entityId, userId, result, context, { triggerSoulReport = false, logCompass = false, compassUserId = userId, skipCompass = false } = {}) {
        let eidosMetricsStr = '';
        ...
        // Internal Compass update (skip when caller handles compass separately, e.g. handlePulseRest)
        if (!skipCompass) {
    ...
    // ==================== DEEP SYNTHESIS COOLDOWN ====================

    /**
     * Check if deep synthesis cooldown has elapsed for an entity
     * @param {string} entityId
     * @returns {boolean}
     */
    _isDeepSynthesisCooldownElapsed(entityId) {
        const lastRun = this.lastDeepSynthesisTime.get(entityId);
        if (!lastRun) return true;
        const cooldownMs = (DEFAULT_CONFIG.deepSynthesis?.cooldownHours || 12) * 60 * 60 * 1000;
        return (Date.now() - lastRun) >= cooldownMs;
    }

    /**
     * Record that deep synthesis ran for an entity
     * @param {string} entityId
     */
    _recordDeepSynthesisRun(entityId) {
        this.lastDeepSynthesisTime.set(entityId, Date.now());
    }
    ...
    async initSession(entityId, userId, forceNew = false, context = {}) {
        try {
            // Single getExpressionState call covers both ensureStructures and getTimeSinceLastInteraction
            const expressionState = await this.hotMemory.getExpressionState(entityId, userId);
            if (!expressionState) {
                await this.hotMemory.updateExpressionState(entityId, userId, createDefaultExpressionState(entityId, userId));
            }

            const lastTs = expressionState?.lastInteractionTimestamp;
            const timeSinceLast = lastTs ? Date.now() - new Date(lastTs).getTime() : null;
            const sessionExpired = timeSinceLast === null || timeSinceLast > 4 * 60 * 60 * 1000;

            if (forceNew || sessionExpired) {
                // Capture episodic buffer BEFORE clearing the session.
                // startNewSession deletes the stream, so we must snapshot it now
                // for compass synthesis to use. This is a fast Redis read (~5ms).
                let capturedBuffer = [];
                if (sessionExpired) {
                    const bufferSize = DEFAULT_CONFIG.internalCompass?.fullBufferForSessionEnd || 30;
                    capturedBuffer = await this.hotMemory.getEpisodicStream(entityId, userId, bufferSize);
                }

                // Clear session state first — episodic stream, active context, expression
                await this.hotMemory.startNewSession(entityId, userId);
                continuityLog.synthesize('session_init', entityId, userId, {});

                // Now run background synthesis using the captured buffer (not re-reading)
                const minTurns = DEFAULT_CONFIG.internalCompass?.minTurnsForSynthesis || 2;

                if (capturedBuffer.length >= minTurns && DEFAULT_CONFIG.internalCompass?.synthesizeOnSessionEnd !== false) {
                    setImmediate(async () => {
                        try {
                            logger.info(`Session expired - synthesizing Internal Compass in background (${capturedBuffer.length} turns)`);
                            continuityLog.synthesize('session_end', entityId, userId, {
                                stats: { turns: capturedBuffer.length }
                            });

                            const compassResult = await this.synthesizer.synthesizeInternalCompass(
                                entityId, userId, capturedBuffer,
                                { aiName: context.aiName || 'Entity', sessionEnding: true }
                            );
                            ...
                            logger.warn(`Background compass synthesis failed: ${compassError.message}`);
                        }
                    });
                }

                // Deep synthesis on session end (doesn't need episodic buffer)
                if (sessionExpired && DEFAULT_CONFIG.deepSynthesis?.runOnSessionEnd !== false) {
                    if (!this._isDeepSynthesisCooldownElapsed(entityId)) {
                        logger.info(`Deep synthesis skipped for ${entityId} — cooldown not elapsed`);
                    } else {
                        this._recordDeepSynthesisRun(entityId);
                        setImmediate(async () => {
                            try {
    ...
    async triggerPulseSynthesis(entityId, context = {}) {
        ...
        setImmediate(async () => {
            try {
                ...
                // Eidos introspection only — skip compass here because
                // handlePulseRest does a full session-end compass with the complete buffer.
                // Running it here too would produce duplicates.
                await this._processEidosAndCompass(
                    entityId,
                    ContinuityMemoryService.PULSE_USER_ID,
                    result,
                    context,
                    { compassUserId: null, skipCompass: true }
                );
            } catch (error) {
    ...
    async handlePulseRest(entityId, context = {}) {
        try {
            ...
            if (recentPulseCount >= 4 && DEFAULT_CONFIG.deepSynthesis?.runOnSessionEnd !== false) {
                if (!this._isDeepSynthesisCooldownElapsed(entityId)) {
                    logger.info(`Pulse deep synthesis skipped for ${entityId} — cooldown not elapsed`);
                } else {
                    this._recordDeepSynthesisRun(entityId);
                    setImmediate(async () => {
                        try {
