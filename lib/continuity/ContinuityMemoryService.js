    ...
    async triggerSynthesis(entityId, userId, context = {}) {
        ...
        setImmediate(async () => {
            try {
                ...
                // Check if Internal Compass needs synthesis (time threshold)
                // This runs periodically during long sessions to maintain temporal narrative
                const needsCompass = await this.synthesizer.needsCompassSynthesis(entityId, userId);
                
                if (needsCompass) {
                    // Get full episodic buffer for compass synthesis
                    const fullBuffer = await this.hotMemory.getEpisodicStream(entityId, userId, 50);
                    
                    if (fullBuffer.length >= (DEFAULT_CONFIG.internalCompass?.minTurnsForSynthesis || 4)) {
                        logger.info(`Triggering Internal Compass synthesis (threshold exceeded)`);
