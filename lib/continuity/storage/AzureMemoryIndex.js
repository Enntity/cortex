...
export class AzureMemoryIndex {
    ...
    /**
     * Parse Azure Search response and deserialize memories
     * @private
     * @param {string} response - Raw JSON response from Azure
    ...
    _parseSearchResults(response) {
        try {
            ...
            logger.warn(`Failed to parse Azure search response: ${error.message}`);
            return [];
        }
    }
     ...
     * @param {number[]} [precomputedEmbedding=null] - Optional pre-computed embedding (avoids regeneration)
     */
            ...
            const results = await Promise.all(
                batch.map(async (memory) => {
                    try {
                        ...
                        // Use existing contentVector - don't regenerate
                        const doc = {
                            id,
                            entityId,
                            userId,
                        ...
                        logger.warn(`Failed to upsert memory in batch: ${error.message}`);
                        return null;
                    }
                })
            );
     ...
     * @param {Object} [options]
     * @param {string} [options.orderBy='importance desc'] - Sort order
     * @returns {Promise<ContinuityMemoryNode[]>}
     */
    async getByType(entityId, userId, type, limit = 50, options = {}) {
        const { orderBy = 'importance desc, lastAccessed desc' } = options;
     ...
     * Returns memories that haven't been through the sleep consolidation cycle.
     * 
     * @param {string} entityId
     * @param {string} userId
     * @param {Object} [options]
     * @param {number} [options.limit=20] - Max memories to return
     * @param {number} [options.skip=0] - Pagination offset
     * @param {string} [options.since] - Only memories after this date (ISO 8601)
    ...
    async getUnprocessedMemories(entityId, userId, options = {}) {
        ...
        try {
            // Build filter: entity + user + NOT sleep-processed tag
            let filter = this._buildBaseFilter(entityId, userId);
            filter += ` and not tags/any(t: t eq 'sleep-processed')`;
            
            // Add date filter if provided
            if (since) {
                filter += ` and timestamp ge ${since}`;
            }
    ...
    async markAsProcessed(memoryId) {
        try {
            const memories = await this.getByIds([memoryId]);
            ...
            // Already processed?
            if (tags.has('sleep-processed')) {
                return true;
            }
            
            tags.add('sleep-processed');
     ...
     * @param {string} entityId
     * @param {string} userId
     * @param {string} [since] - Optional date filter
    ...
    async getPromotionCandidates(entityId, userId, options = {}) {
        ...
        try {
            let filter = this._buildBaseFilter(entityId, userId);
            filter += ` and type eq 'IDENTITY'`;
            filter += ` and tags/any(t: t eq 'promotion-candidate')`;
     ...
     * @param {string} userId
     * @param {Object} [options]
     * @param {number} [options.limit=100] - Max to return
     * @returns {Promise<ContinuityMemoryNode[]>}
     */
    async getCoreExtensions(entityId, userId, options = {}) {
        ...
        try {
            let filter = this._buildBaseFilter(entityId, userId);
            filter += ` and type eq 'CORE_EXTENSION'`;
     ...
     * Used by deterministic promotion process after validation
     * @param {string} memoryId
     * @returns {Promise<boolean>}
     */
    async promoteToCore(memoryId) {
        ...
        try {
            ...
            await this.upsertMemory({
                ...
                type: 'CORE_EXTENSION',
                tags: updatedTags,
                importance: Math.max(memory.importance || 5, 8)  // Boost importance
            });
