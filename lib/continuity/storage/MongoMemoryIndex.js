...
export class MongoMemoryIndex {
     ...
     * entity-level memories (assocEntityIds: []) so the entity can recall
    ...
    _buildBaseFilter(entityId, assocEntityId) {
        ...
        // Include both user-scoped memories and entity-level memories (assocEntityIds: [])
        // This ensures the entity's own experiences are visible during user conversations
        return {
            entityId,
            $or: [
    ...
    _buildEntityLevelFilter(entityId) {
        return { entityId, assocEntityIds: [] };
    }
    ...
    async upsertMemory(entityId, userId, memory) {
        ...
        try {
            ...
            // Otherwise, memories are associated with the specific user
            const assocEntityIds = this._isEntityLevelType(memoryType) || !userId ? [] : [userId];

            if (!this._isEntityLevelType(memoryType) && assocEntityIds.length === 0) {
    ...
    async upsertMemories(entityId, userId, memories) {
        ...
        try {
            ...
            const operations = memories.map(memory => {
                ...
                // CORE memories are entity-level (shared across all users)
                const assocEntityIds = this._isEntityLevelType(memoryType) ? [] : [userId];
    ...
    async getByType(entityId, userId, type, limit = 50, options = {}) {
        try {
            ...
            if (this._isEntityLevelType(type)) {
                ...
                filter = { entityId, assocEntityIds: [], type };
            } else if (!userId) {
    ...
    async getCoreExtensions(entityId, userId, options = {}) {
        ...
        try {
            ...
            const results = await collection
                .find({ entityId, assocEntityIds: [], type: 'CORE_EXTENSION' })
                .sort({ importance: -1 })
                .limit(limit)
                .toArray();
        ...
        try {
            ...
            await collection.updateOne(
                { id: memoryId },
                {
                    $set: {
                        type: 'CORE_EXTENSION',
                        // CORE_EXTENSION is entity-level - clear user associations
                        assocEntityIds: [],
                        tags: updatedTags,
