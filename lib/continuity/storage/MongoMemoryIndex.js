...
export class MongoMemoryIndex {
    ...
    async _getCollection() {
        ...
        try {
            ...
            // Ensure indexes exist (fire-and-forget on first connect)
            if (!this._indexesEnsured) {
                this._ensureIndexes().catch(err =>
    ...
    /**
     * Ensure required indexes exist on the continuity_memories collection.
     * Uses createIndex which is a no-op if the index already exists.
     * Called once per connection (fire-and-forget).
     * @private
     */
    async _ensureIndexes() {
        ...
        await Promise.all([
            // Unique index on id — used by every single-doc read/write/delete
            col.createIndex({ id: 1 }, { unique: true, name: 'idx_id' }),

            // Compound: entity + assocEntityIds + type — covers getByType, getCoreExtensions,
            // getPromotionCandidates, vector search pre-filters, countDocuments, getAllMemories
            col.createIndex(
                { entityId: 1, assocEntityIds: 1, type: 1 },
                { name: 'idx_entity_assoc_type' }
            ),

            // Compound: entityId + type — covers _buildBaseFilter with no userId (entityId-only),
            // and queries that filter by entityId + type without assocEntityIds
            col.createIndex(
                { entityId: 1, type: 1 },
                { name: 'idx_entity_type' }
            ),

            // Tags — covers promotion-candidate lookups and tag-based filters
            col.createIndex(
                { tags: 1 },
                { name: 'idx_tags' }
            ),

            // Text index on content + tags — used by searchFullText() for $text queries
            col.createIndex(
                { content: 'text', tags: 'text' },
                { name: 'idx_text_search', weights: { content: 10, tags: 5 } }
            ),
        ]);

        // Drop stale indexes from pre-assocEntityIds schema and orphaned indexes
        const staleIndexes = [
            'idx_entity_user_type',
            'idx_entity_user_importance',
            'idx_entity_user_tags',
            'idx_entity_user',
            'idx_timestamp',  // Orphaned — not in code, causes suboptimal query plans
        ];
        for (const name of staleIndexes) {
            try {
                await col.dropIndex(name);
                logger.info(`Dropped stale index: ${name}`);
            } catch {
                // Index doesn't exist — fine
            }
        }

        logger.info(`Indexes ensured on ${this.collectionName} in ${Date.now() - start}ms`);
    }
    ...
    async _buildClientOptions() {
        // Base TLS options for MongoDB Atlas compatibility
        const baseOptions = {
            tls: true,
        ...
        // If no encryption key, return base TLS options only
        ...
        try {
            ...
            // Merge TLS options with CSFLE options
            return {
                ...baseOptions,
    ...
    /**
     * Memory types that are entity-level (not filtered by userId)
     * CORE = Fundamental identity shared across all users
     * CORE_EXTENSION = Hardened patterns promoted from identity evolution
     * Both represent who the entity IS, regardless of who they're talking to
     */
    static ENTITY_LEVEL_TYPES = ['CORE', 'CORE_EXTENSION'];

    /**
     * Check if a memory type is entity-level (shared across all users)
     * @private
     * @param {string} type
     * @returns {boolean}
     */
    _isEntityLevelType(type) {
        return MongoMemoryIndex.ENTITY_LEVEL_TYPES.includes(type);
    }

    /**
     * Build a base filter for entity and associated entities.
     * When a userId is provided, returns BOTH user-scoped memories AND
     * entity-level memories (assocEntityIds: [entityId]) so the entity can recall
     * its own experiences (e.g. life loop events) during user conversations.
     * @private
     * @param {string} entityId
     * @param {string} assocEntityId - Single associated entity ID to filter by
     *                                 If null/undefined/empty, searches ALL memories for this entity
     * @returns {Object}
     */
    _buildBaseFilter(entityId, assocEntityId) {
        // Null, undefined, or empty string = search all memories for this entity
        if (!assocEntityId) {
            return { entityId };
        }
        // Include both user-scoped memories and entity-level memories
        // Entity-level memories use entityId as sentinel in assocEntityIds (not empty array)
        // so that Atlas Vector Search pre-filters work (they require scalar values)
        return {
            entityId,
            $or: [
                { assocEntityIds: assocEntityId },
                { assocEntityIds: entityId },
            ]
        };
    }

    /**
     * Build an entity-level filter (for CORE/CORE_EXTENSION memories)
     * @private
     * @param {string} entityId
     * @returns {Object}
     */
    _buildEntityLevelFilter(entityId) {
        return { entityId, assocEntityIds: entityId };
    }
    ...
    async searchFullText(entityId, userId, query, options = {}) {
        ...
        try {
            ...
            // Exclude memories with specific tags (e.g., 'deep-processed')
            if (options.excludeTags?.length) {
                filter.tags = { ...filter.tags, $nin: options.excludeTags };
            }
    ...
    async upsertMemory(entityId, userId, memory) {
        ...
        try {
            ...
            const memoryType = memory.type || ContinuityMemoryType.ANCHOR;

            // CORE memories are entity-level (shared across all users)
            // When no userId is provided (e.g. life loop events), memories are entity-level
            // Entity-level memories use entityId as sentinel (not empty array) for Atlas Vector Search compatibility
            const assocEntityIds = this._isEntityLevelType(memoryType) || !userId ? [entityId] : [userId];

            if (!this._isEntityLevelType(memoryType) && assocEntityIds[0] === entityId) {
                logger.debug(`Entity-level ${memoryType} memory stored for ${entityId} (no user association)`);
            }
    ...
    async upsertMemories(entityId, userId, memories) {
        ...
        try {
            ...
            const operations = memories.map(memory => {
                ...
                // CORE memories are entity-level (shared across all users)
                // Entity-level memories use entityId as sentinel for Atlas Vector Search compatibility
                const assocEntityIds = this._isEntityLevelType(memoryType) ? [entityId] : [userId];

                // Safety check: warn if a non-entity-level type would be stored without user association
                if (!this._isEntityLevelType(memoryType) && assocEntityIds[0] === entityId) {
                    logger.warn(`Memory type ${memoryType} being saved without user association for entity ${entityId} - this may expose user-scoped data`);
                }
    ...
    async getByType(entityId, userId, type, limit = 50, options = {}) {
        try {
            ...
            let filter;
            if (this._isEntityLevelType(type)) {
                // CORE/CORE_EXTENSION are entity-level (shared across all users)
                filter = { entityId, assocEntityIds: entityId, type };
            } else if (!userId) {
                // Null/undefined/empty userId = search all memories of this type
                filter = { entityId, type };
            } else {
                // User-scoped + entity-level memories of this type
                filter = {
                    entityId,
                    type,
                    $or: [
    ...
    /**
     * Mark multiple memories as deep-processed in a single batch operation
     * @param {string[]} memoryIds
     * @returns {Promise<number>} Number of memories tagged
     */
    async markDeepProcessed(memoryIds) {
        if (!memoryIds?.length) return 0;
        try {
            const collection = await this._getCollection();
            const result = await collection.updateMany(
                { id: { $in: memoryIds } },
                { $addToSet: { tags: 'deep-processed' } }
            );
            return result.modifiedCount;
        } catch (error) {
            logger.error(`Failed to mark memories as deep-processed: ${error.message}`);
            return 0;
        }
    }
     ...
     * CORE_EXTENSION is entity-level (shared across all users)
     * @param {string} entityId
     * @param {string} userId - Unused, kept for API compatibility
     * @param {Object} [options]
     * @returns {Promise<ContinuityMemoryNode[]>}
     */
    async getCoreExtensions(entityId, userId, options = {}) {
        ...
        try {
            ...
            // CORE_EXTENSION is entity-level (no userId filter)
            const results = await collection
                .find({ entityId, assocEntityIds: entityId, type: 'CORE_EXTENSION' })
                .sort({ importance: -1 })
                .limit(limit)
                .toArray();
        ...
        try {
            ...
            await collection.updateOne(
                { id: memoryId },
                {
                    $set: {
                        type: 'CORE_EXTENSION',
                        // CORE_EXTENSION is entity-level - use entityId as sentinel
                        assocEntityIds: [memory.entityId],
                        tags: updatedTags,
            ...
            logger.info(`Promoted memory ${memoryId} to CORE_EXTENSION (now entity-level)`);
            return true;
        } catch (error) {
     ...
     * @param {string} entityId
     * @param {string} userId - If null/undefined/empty, checks all memories for entity
     * @returns {Promise<boolean>}
     */
     ...
     * @param {string} entityId
     * @param {string} userId - If null/undefined/empty, counts all memories for entity
     * @returns {Promise<number>}
     */
     ...
     * @param {string} entityId
     * @param {string} userId - If null/undefined/empty, gets all memories for entity
     * @param {Object} [options]
     * @returns {Promise<ContinuityMemoryNode[]>}
     */
        ...
        return results
            .map(result => {
                ...
                if (queryEmbedding && deserialized.contentVector && Array.isArray(deserialized.contentVector)) {
                    const trueSimilarity = cosineSimilarity(queryEmbedding, deserialized.contentVector);
                    if (trueSimilarity > 0) {
