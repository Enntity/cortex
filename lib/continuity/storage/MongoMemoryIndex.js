 ...
 * 
 * This is a parallel implementation to AzureMemoryIndex that can be swapped in
 * when migrating from Azure AI Search to MongoDB Atlas.
 */
...
export class MongoMemoryIndex {
    ...
    async _getCollection() {
        ...
        try {
            ...
            logger.info(`Connected to MongoDB: ${this._db.databaseName}.${this.collectionName}`);
            return this._collection;
        } catch (error) {
    ...
    async getCoreExtensions(entityId, userId, options = {}) {
        ...
        try {
            ...
            const results = await collection
                .find({ entityId, userId, type: 'CORE_EXTENSION' })
                .sort({ importance: -1 })
                .limit(limit)
                .toArray();
    ...
    async hasMemories(entityId, userId) {
        try {
            const collection = await this._getCollection();
            const count = await collection.countDocuments({ entityId, userId }, { limit: 1 });
            return count > 0;
        } catch (error) {
    ...
    async cascadingForget(entityId, userId) {
        try {
            ...
            const allMemories = await collection.find({ entityId, userId }).toArray();
    ...
    async getMemoryCount(entityId, userId) {
        try {
            const collection = await this._getCollection();
            return await collection.countDocuments({ entityId, userId });
        } catch (error) {
     ...
     * MongoDB stores objects natively, so this is simpler than Azure
     * @private
     */
