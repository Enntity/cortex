...
export class MongoMemoryIndex {
    ...
    async promoteToCore(memoryId) {
        ...
        try {
            ...
            // Only clear user associations for entity-level memory types
            const ENTITY_LEVEL_TYPES = ['CORE', 'CORE_EXTENSION'];
            if (!ENTITY_LEVEL_TYPES.includes('CORE_EXTENSION')) {
                // For user-scoped types, preserve the association
                // This prevents accidentally making user memories visible to all users
                logger.warn(`Attempted to promote memory ${memoryId} to non-entity-level type`);
                return false;
            }
