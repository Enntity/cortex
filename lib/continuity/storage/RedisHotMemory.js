    ...
    async updateEidosMetrics(entityId, userId, metrics) {
        ...
        try {
            ...
            if (metrics.turnCount !== undefined) {
                // Stored unencrypted for atomic HINCRBY (system encryption would break HINCRBY).
                await this.client._raw.hset(key, 'turnCount', String(metrics.turnCount));
                hasPlainTurnCount = true;
            }
            ...
            if (Object.keys(data).length > 0 || hasPlainTurnCount) {
                await this.client._raw.expire(key, 60 * 60 * 24 * 30); // 30 days
            }
        } catch (error) {
