...
// Consolidation drift thresholds — how close a synthesized consolidation must stay
// to its source memories. Empirical data showed 86% of consolidations failed with
// scores in the 0.61–0.84 range at the old thresholds (0.80/0.70).
// Relaxed to capture legitimate consolidations while rejecting truly drifted ones.
const CONSOLIDATION_CENTROID_THRESHOLD = 0.70;  // Was 0.80
const CONSOLIDATION_SOURCE_THRESHOLD = 0.60;    // Was 0.70
...
export class NarrativeSynthesizer {
    ...
    async synthesizeTurn(entityId, userId, episodicBuffer, context = {}) {
        ...
        try {
            ...
            // Pass through Eidos authenticity assessment
            if (synthesis.authenticityAssessment) {
                result.authenticityAssessment = synthesis.authenticityAssessment;
            }

            // Pass through Eidos voice check
            if (synthesis.voiceCheck) {
                result.voiceCheck = synthesis.voiceCheck;
            }
    ...
    async runDeepSynthesis(entityId, userId, options = {}) {
        ...
        try {
                ...
                if (maxMemories >= PAGE_LIMIT) {
                    ...
                    while (hasMore && recentMemories.length < maxMemories) {
                        ...
                        const pageMemories = await this.memoryIndex.searchFullText(
                            ...
                            userId,
                            '*',
                            { limit: fetchLimit, skip, since: sinceDate, sort: { timestamp: -1 }, excludeTags: ['deep-processed'] }
                        );
                    ...
                    userId,
                    '*',
                        { limit: maxMemories, since: sinceDate, sort: { timestamp: -1 }, excludeTags: ['deep-processed'] }
                );
                }
            }
            ...
            const analysis = this._parseSynthesisResponse(response);
                ...
                // Mark batch memories as deep-processed so they're skipped in future runs
                const batchIds = batchMemories.map(m => m.id);
                await this.memoryIndex.markDeepProcessed(batchIds);
            }
    ...
    async _consolidateShorthands(entityId, userId) {
        ...
        try {
            // Fetch all shorthand memories by synthesisType or tag
            // Use searchAllWithFilter to avoid $text search which fails with CSFLE encryption
            const shorthandFilter = {
                entityId,
                assocEntityIds: userId,
                $or: [
                    { synthesisType: 'shorthand' },
                    { tags: 'shorthand' }
                ]
            };
            const shorthandResults = await this.memoryIndex.searchAllWithFilter(
                shorthandFilter,
                { limit: 200 }
            );
            ...
            for (const s of shorthandResults) {
    ...
    async _processDeepAnalysisBatch(entityId, userId, analysis, stats) {
        ...
        for (const consolidation of analysis.consolidations || []) {
            if (consolidation.sourceIds?.length > 1 && consolidation.synthesizedContent) {
                ...
                if (simToCentroid < CONSOLIDATION_CENTROID_THRESHOLD || minSimToSource < CONSOLIDATION_SOURCE_THRESHOLD) {
                ...
                // If nominated for promotion, try re-nominating an existing candidate first
                if (consolidation.nominateForPromotion) {
                    const reNominated = await this._reNominateExisting(
                        entityId, userId, consolidation.synthesizedContent
                    );
                    if (!reNominated) {
                        tags.push('promotion-candidate', `nominated-${Date.now()}`);
                        logger.info(`New nomination for promotion: "${consolidation.synthesizedContent.substring(0, 80)}..."`);
                    }
                    stats.nominations = (stats.nominations || 0) + 1;
                }

                // Store as IDENTITY if newly nominated, otherwise ARTIFACT
                const memoryType = consolidation.nominateForPromotion && tags.includes('promotion-candidate')
        ...
        for (const pattern of analysis.patterns || []) {
            if (pattern.content) {
                ...
                if (pattern.nominateForPromotion) {
                    const reNominated = await this._reNominateExisting(
                        entityId, userId, pattern.content
                    );
                    if (!reNominated) {
                        tags.push('promotion-candidate', `nominated-${Date.now()}`);
                        logger.info(`New nomination for promotion: "${pattern.content.substring(0, 80)}..."`);
                    }
                    stats.nominations = (stats.nominations || 0) + 1;
                }

                // Store as IDENTITY if newly nominated, otherwise ARTIFACT
                const memoryType = pattern.nominateForPromotion && tags.includes('promotion-candidate')
                    ? ContinuityMemoryType.IDENTITY
                    : ContinuityMemoryType.ARTIFACT;
    ...
    async _processIdentityEvolution(entityId, userId, evolutions, result) {
        ...
        for (const evolution of evolutions) {
            ...
            let importance = evolution.importance || 6;
            ...
            // Eidos: annotate with alignment flag if present
            if (evolution.alignmentFlag) {
                memory.metadata = {
                    ...memory.metadata,
                    alignmentFlag: evolution.alignmentFlag,
                };
                tags.push('eidos-flagged');
                // Reduce importance for accommodation-flagged memories
                if (evolution.alignmentFlag === 'accommodation') {
                    memory.importance = Math.round(memory.importance * 0.7);
                }
            }
     ...
     * 4. Not semantically duplicate of a stronger candidate (> 0.85 similarity)
     *
     * @param {string} entityId
     * @param {string} userId
    ...
    async processPromotionCandidates(entityId, userId) {
        ...
        const stats = { candidates: 0, promoted: 0, rejected: 0, deferred: 0, mergedDuplicates: 0 };
        ...
        try {
            ...
            // Dedup candidates against each other before promotion
            const dedupedCandidates = await this._dedupCandidates(entityId, userId, candidates, MAX_SIMILARITY);
            stats.mergedDuplicates = candidates.length - dedupedCandidates.length;

            if (stats.mergedDuplicates > 0) {
                logger.info(`Deduped candidates: ${candidates.length} → ${dedupedCandidates.length} (merged ${stats.mergedDuplicates} duplicates)`);
            }
            ...
            logger.info(`Processing ${dedupedCandidates.length} promotion candidates against ${existingCoreExt.length} existing CORE_EXTENSIONs (min nominations: ${MIN_NOMINATIONS})`);
            ...
            for (const candidate of dedupedCandidates) {
                ...
                if (isDuplicate) {
                    ...
                    await this._demoteCandidate(candidate, 'promotion-rejected');
                    stats.rejected++;
                    continue;
                }
            ...
            logger.info(`Promotion processing complete: ${stats.promoted} promoted, ${stats.rejected} rejected, ${stats.deferred} deferred, ${stats.mergedDuplicates} merged duplicates`);
            return stats;
    ...
    /**
     * Dedup promotion candidates against each other.
     *
     * Sorts by nomination count (strongest first), then walks through candidates.
     * If a weaker candidate is too similar to a stronger one, merge its nomination
     * tags into the stronger candidate and demote the weaker one.
     *
     * @private
     * @param {string} entityId
     * @param {string} userId
     * @param {Object[]} candidates - All promotion candidates
     * @param {number} threshold - Similarity threshold (same as CORE_EXTENSION dedup)
     * @returns {Promise<Object[]>} Surviving candidates with merged nominations
     */
    async _dedupCandidates(entityId, userId, candidates, threshold) {
        if (candidates.length <= 1) return candidates;

        // Sort by nomination count descending — strongest candidates survive
        const sorted = [...candidates].sort((a, b) => {
            const aNoms = (a.tags || []).filter(t => t.startsWith('nominated-')).length;
            const bNoms = (b.tags || []).filter(t => t.startsWith('nominated-')).length;
            return bNoms - aNoms;
        });

        const surviving = [];  // Accepted candidates (not merged away)
        const mergedIds = new Set();  // IDs of candidates merged into others

        for (const candidate of sorted) {
            if (mergedIds.has(candidate.id)) continue;

            // Check against all surviving candidates via vector search
            // Use stored embedding to avoid re-embedding API calls
            let isDuplicate = false;
            if (surviving.length > 0 && candidate.content) {
                try {
                    const similar = await this.memoryIndex.searchSemantic(
                        entityId, userId, candidate.content, 5, ['IDENTITY'],
                        candidate.contentVector
                    );

                    for (const match of similar) {
                        if ((match._vectorScore || 0) < threshold) break;
                        if (!match.tags?.includes('promotion-candidate')) continue;
                        if (match.id === candidate.id) continue;

                        // Is this match one of our surviving candidates?
                        const survivor = surviving.find(s => s.id === match.id);
                        if (!survivor) continue;

                        // Merge: transfer nomination tags to the stronger candidate
                        const candidateNomTags = (candidate.tags || []).filter(t => t.startsWith('nominated-'));
                        const survivorNomTags = new Set((survivor.tags || []).filter(t => t.startsWith('nominated-')));

                        // Add candidate's nomination timestamps that the survivor doesn't have
                        let added = 0;
                        for (const tag of candidateNomTags) {
                            if (!survivorNomTags.has(tag)) {
                        ...
                        if (added > 0) {
                            // Persist the merged tags to the survivor
                            await this.memoryIndex.upsertMemory(entityId, userId, {
                                ...survivor,
                                tags: survivor.tags
                            });
                        }

                        const totalNoms = survivor.tags.filter(t => t.startsWith('nominated-')).length;
                        logger.info(`Merged candidate ${candidate.id} into ${survivor.id} (+${added} nominations, now ${totalNoms}): "${candidate.content?.substring(0, 60)}..."`);

                        // Demote the weaker candidate
                    ...
                    logger.warn(`Candidate dedup check failed for ${candidate.id}: ${error.message}`);
                }
            }
    ...
    /**
     * Re-nominate an existing promotion candidate instead of creating a duplicate.
     *
     * When deep synthesis nominates a pattern, the LLM often rephrases it enough
     * that the deduplicator (0.85 threshold) doesn't catch it. This method uses a
     * lower threshold (0.70) to find existing candidates with the same theme and
     * adds a nomination tag to them instead.
     *
     * @private
     * @param {string} entityId
     * @param {string} userId
     * @param {string} content - The nominated content
     * @returns {Promise<boolean>} True if an existing candidate was re-nominated
     */
    async _reNominateExisting(entityId, userId, content) {
        const RE_NOMINATION_THRESHOLD = 0.70;

        try {
            const similar = await this.memoryIndex.searchSemantic(
                entityId, userId, content, 5, ['IDENTITY']
            );

            for (const match of similar) {
                if ((match._vectorScore || 0) < RE_NOMINATION_THRESHOLD) break;
                if (!match.tags?.includes('promotion-candidate')) continue;

                const updatedTags = [...(match.tags || []), `nominated-${Date.now()}`];
                await this.memoryIndex.upsertMemory(entityId, userId, {
                    ...match,
                    tags: updatedTags
                });

                const nominationCount = updatedTags.filter(t => t.startsWith('nominated-')).length;
                logger.info(`Re-nominated existing candidate ${match.id} (now ${nominationCount} nominations): "${content.substring(0, 60)}..."`);
                return true;
            }
            return false;
        } catch (error) {
            logger.warn(`Re-nomination check failed: ${error.message}`);
            return false;
        }
    }
     ...
     * @private
     * @param {Object} memory - The candidate memory
     */
    async _demoteCandidate(memory) {
        try {
            ...
            await this.memoryIndex.upsertMemory(memory.entityId, memory.assocEntityIds?.[0] || null, {
                ...memory,
                tags: cleanedTags
            });
        } catch (error) {
    ...
    async synthesizeInternalCompass(entityId, userId, episodicBuffer, context = {}) {
        const { aiName = 'Entity', sessionEnding = false, eidosMetrics = '' } = context;
