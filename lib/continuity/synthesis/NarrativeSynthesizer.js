    ...
    async _applySleepDecision(entityId, userId, fresh, decision, stats) {
        ...
        switch (action) {
            ...
            case 'MERGE': {
                ...
                if (!driftCheck.valid) {
                    // Merge would cause semantic drift - fall back to LINK
                    logger.info(`MERGE drift check FAILED for ${fresh.id}: sim(M',M)=${driftCheck.mergedToM.toFixed(3)} < ${driftCheck.minSimToM.toFixed(3)} OR sim(M',S)=${driftCheck.mergedToS.toFixed(3)} < ${driftCheck.originalSim.toFixed(3)}. Linking instead.`);
