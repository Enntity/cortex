...
// 0.75 = similar enough to merge (relaxed for near-matches)
...
// Time window for in-memory cache (ms) - helps with Azure indexing delay
...
export class MemoryDeduplicator {
    ...
    constructor(memoryIndex, options = {}) {
        ...
        // In-memory cache for recently stored memories (workaround for Azure indexing delay)
        // Key: entityId:userId:type, Value: { memories: [], timestamp: Date }
        this.recentMemoryCache = new Map();
    }
    
    /**
     * Add to recent memory cache (for immediate dedup before Azure indexes)
     * @private
     */
    ...
    /**
     * Check cache for recent similar memories (before Azure indexes them)
     * @private
     */
    _checkCache(entityId, userId, content, type) {
        ...
        for (const cached of cacheEntry.memories) {
            ...
            if (similarity >= 0.85) { // High threshold for cache matches (quick text comparison)
                logger.debug(`Found cached recent memory with similarity ${similarity.toFixed(3)}: ${cached.id}`);
                return cached;
            }
        }
     ...
     * 
     * Uses both:
     * 1. In-memory cache (for rapid-fire requests before Azure indexes)
     * 2. Azure vector search (for indexed memories)
     * 
     ...
     */
    async storeWithDedup(entityId, userId, memory) {
        if (!memory.content) {
            logger.warn('Cannot deduplicate memory without content');
            return { id: null, merged: false, mergedCount: 0 };
        }
        
        try {
            // FIRST: Check in-memory cache for very recent stores (Azure indexing delay workaround)
            const cachedMatch = this._checkCache(entityId, userId, memory.content, memory.type);
            if (cachedMatch) {
                logger.info(`Found duplicate in recent cache (before Azure indexed). Skipping duplicate store for: "${memory.content.substring(0, 60)}..."`);
                ...
                return { 
                    ...
                    mergedCount: 1,
                    mergedIds: [],
                    message: 'Duplicate detected from recent store (Azure indexing delay bypass)'
                };
            }
            
            // SECOND: Find semantically similar existing memories in Azure
            const similarMemories = await this._findSimilarMemories(
                entityId, userId, memory.content, memory.type
            );
            
            if (similarMemories.length === 0) {
                // No duplicates - store normally and add to cache
                const id = await this.memoryIndex.upsertMemory(entityId, userId, memory);
                logger.debug(`No duplicates found, stored new memory: ${id}`);
                
                // Add to cache for rapid-fire dedup
                this._addToCache(entityId, userId, { ...memory, id });
                
                return { id, merged: false, mergedCount: 0 };
            }
            
            logger.info(`Found ${similarMemories.length} similar memory(ies) to merge. Content preview: "${memory.content.substring(0, 100)}..."`);
            ...
            logger.error(`Deduplication failed: ${error.message}`, error);
    ...
    /**
     * Find vector-similar memories using Azure Search
     * Uses vector embeddings to find semantically similar content
     * @private
     */
    async _findSimilarMemories(entityId, userId, content, type = null) {
        try {
            ...
            const similar = vectorResults.filter(m => {
                ...
                // Azure AI Search vector scores for cosine similarity are already in ~0-1 range
                ...
                if (rawVectorScore > 0.7 || isSimilar) {
                    logger.debug(`Vector match: score=${rawVectorScore.toFixed(3)}, recall=${(m._recallScore || 0).toFixed(3)}, threshold=${this.similarityThreshold}, similar=${isSimilar}`);
                }
            ...
            if (similar.length > 0) {
                ...
                logger.debug(`No memories met similarity threshold ${this.similarityThreshold}. Top result score: ${vectorResults[0]?.['@search.score']?.toFixed(3) ?? 'N/A'}`);
            }
            
            return similar.slice(0, this.maxClusterSize);
        } catch (error) {
            logger.error(`Failed to find similar memories: ${error.message}`, error);
            return [];
        }
    }
    ...
    async _mergeMemories(newMemory, existingMemories) {
        ...
        // Importance: boost based on frequency, cap at 10
        const baseImportance = Math.max(...allMemories.map(m => m.importance || 5));
        const frequencyBoost = Math.min(allMemories.length * 0.5, 2); // Max +2 boost
        const resolvedImportance = Math.min(10, Math.round(baseImportance + frequencyBoost));
        ...
        allTags.add('merged'); // Mark as merged
        allTags.add('auto-dedup');
    ...
    async _callSynthesisForMerge(contents) {
        const prompt = `You are consolidating similar memories into one coherent memory entry.
These memories capture related but slightly different perspectives on the same insight.

Memories to consolidate:
${contents.map((c, i) => `${i + 1}. ${c}`).join('\n')}

Create a single, concise memory that:
- Captures the essential meaning from all versions
- Preserves any unique details or nuances
- Is written as a cohesive narrative insight
- Does NOT start with "User" or describe the memory format

Return ONLY the consolidated memory content, nothing else.`;

        try {
            const response = await callPathway('chat', {
                chatHistory: [],
                contextId: `dedup-${Date.now()}`,
                systemPrompt: 'You are a memory consolidation assistant. Return only the consolidated memory text.',
                text: prompt,
