...
// Skip LLM merge when new and existing memories are this similar.
// At >= 0.95 cosine similarity, the content is near-identical and the LLM merge
// just rephrases without adding value. Absorb instead: keep new, delete existing.
...
export class MemoryDeduplicator {
    ...
    async storeWithDedup(entityId, userId, memory, options = {}) {
        ...
        try {
            ...
            // OPTIMIZATION: Skip LLM merge when content is near-identical (>= 0.95 similarity).
            // The LLM merge would just rephrase without adding value — absorb instead.
            const bestMatch = similarMemories[0];
            const bestMatchVector = bestMatch.contentVector || [];
            if (bestMatchVector.length > 0 && memory.contentVector.length > 0) {
                const directSim = cosineSimilarity(memory.contentVector, bestMatchVector);
                if (directSim >= ABSORB_SIMILARITY_THRESHOLD) {
                    logger.info(`Near-identical content (sim=${directSim.toFixed(3)} >= ${ABSORB_SIMILARITY_THRESHOLD}). Absorbing without LLM merge.`);

                    // Delete existing similar memories (absorbed by the new one)
                    const idsToDelete = similarMemories.map(m => m.id).filter(Boolean);
                    for (const id of idsToDelete) {
                        try {
                            await this.memoryIndex.deleteMemory(id);
                        } catch (err) {
                            logger.warn(`Failed to delete absorbed memory ${id}: ${err.message}`);
                        }
                    }

                    // Store M as-is (no LLM merge needed)
                    const id = await this.memoryIndex.upsertMemory(entityId, userId, memory);

                    this._addToCache(entityId, userId, { ...memory, id });
                    this._notifyMemoryWrite(entityId, userId);

                    continuityLog.synthesize('memory_absorb', entityId, userId, {
                        content: memory.content,
                        stats: { absorbed: similarMemories.length, type: memory.type, skippedMerge: true }
                    });

                    return { id, merged: true, mergedCount: similarMemories.length, mergedIds: idsToDelete, absorbed: true, skippedMerge: true };
                }
            }
            ...
            if (!driftCheck.valid) {
                // Bad merge detected - decide what to do based on the reason
                if (driftCheck.reason === 'did_not_incorporate') {
                    // The LLM just rephrased M without incorporating S
                    // Since M and S are similar (>0.85), M already captures S's info
                    // ABSORB: keep M as-is, delete S (the similar existing memory)
                    logger.info(`Merge drift check: did_not_incorporate (M' not closer to S). sim(M',M)=${driftCheck.mergedToM.toFixed(3)}, sim(M',S)=${driftCheck.mergedToS.toFixed(3)}, sim(M,S)=${driftCheck.originalSim.toFixed(3)}. Absorbing - keeping M, deleting similar.`);
                    
                    // Delete the similar existing memory (S) - M supersedes it
                    const idsToDelete = similarMemories.map(m => m.id).filter(Boolean);
                    for (const id of idsToDelete) {
                        try {
                            await this.memoryIndex.deleteMemory(id);
                    ...
                    // Store M as-is (not the merged version)
                    const id = await this.memoryIndex.upsertMemory(entityId, userId, memory);
                    ...
                    continuityLog.synthesize('memory_absorb', entityId, userId, {
                        content: memory.content,
                        stats: { absorbed: similarMemories.length, type: memory.type }
                    });
                    
                    return { id, merged: true, mergedCount: similarMemories.length, mergedIds: idsToDelete, absorbed: true };
                } else {
                    // The merge pulled too far toward S, losing M's info
                    // LINK: keep both separate, they're meaningfully different
                    logger.info(`Merge drift check: pulled_toward_existing (M' closer to S than M). sim(M',M)=${driftCheck.mergedToM.toFixed(3)}, sim(M',S)=${driftCheck.mergedToS.toFixed(3)}. Linking instead.`);
