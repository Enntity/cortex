...
export class ContextBuilder {
    ...
    buildContextWindow({
        ...
        logger.debug(`Building context window: ${relevantMemories.length} relevant memories, ${expandedMemories.length} expanded memories`);
        if (relevantMemories.length > 0) {
            const types = relevantMemories.map(m => m.type).join(', ');
            logger.debug(`Relevant memory types: ${types}`);
        }
        ...
        if (anchorsSection) {
            logger.debug(`Built anchors section with ${anchorsSection.split('\n').length - 1} anchors`);
        ...
        if (vocabularySection) {
            logger.debug(`Built vocabulary section with ${vocabularySection.split('\n').length - 2} terms`);
     ...
     * 
     * These are fundamental identity and behavior directives that should
     * always be present regardless of the current topic. They form the
    ...
    _buildCoreSection(relevantMemories, expandedMemories) {
        ...
        const coreMemories = allMemories.filter(m => m.type === ContinuityMemoryType.CORE);
        ...
        const items = sorted.map(m => `- ${m.content}`);
    ...
    _buildVocabularySection(relevantMemories, expandedMemories) {
        ...
        logger.debug(`Vocabulary section: found ${shorthands.length} shorthand memories from ${allMemories.length} total`);
        ...
        for (const memory of shorthands) {
            ...
            if (relContext?.sharedVocabulary) {
                for (const [term, meaning] of Object.entries(relContext.sharedVocabulary)) {
                    ...
                    logger.debug(`Extracted vocabulary from relationalContext: "${term}"`);
                }
            }
        ...
        for (const memory of allMemories) {
            if (memory.content) {
                ...
                if (match && !vocabulary.has(match[1])) {
                    vocabulary.set(match[1], match[2]);
                    logger.debug(`Detected vocabulary from content: "${match[1]}" → "${match[2].substring(0, 50)}..."`);
                }
            }
        }
    ...
    async generateNarrativeSummary(entityId, userId, memories, currentQuery) {
        ...
        try {
            ...
            const response = await callPathway('continuity_narrative_summary', {
                currentQuery,
                memoriesText
            });
    ...
    _formatMemoriesForSummary(memories) {
        ...
        for (const [type, mems] of Object.entries(grouped)) {
            ...
            const items = mems.slice(0, 5).map(m => {
                let text = `- ${m.content}`;
