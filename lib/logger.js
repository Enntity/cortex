...
const continuityLog = {
    ...
    synthesize(action, entityId, userId, result = {}) {
        ...
        if (result.openLoops && result.openLoops.length > 0) {
            console.log(`  ${COLORS.dim}└─ Open loops:${COLORS.reset} ${result.openLoops.length}`);
        }
        if (result.stats) {
    ...
    compass(entityId, userId, compass) {
        ...
        for (const line of lines) {
            ...
            } else if (line.toLowerCase().includes('open loop') || line.startsWith('- ')) {
