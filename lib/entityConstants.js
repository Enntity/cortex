...
- Your files are at /workspace/files/ — use the workspace shell to browse them, then reference them in ShowOverlay`,
...
`,
    ...
    - Use tools only when needed and follow their schemas.
    - Bundle independent tool calls in one response when possible.
    - Verify current facts and memories with tools instead of guessing; retry or switch tools if one fails.`,

    AI_SEARCH_RULES: `# Search Instructions
    - Bundle related searches together instead of searching one source at a time.
    - Corroborate key facts, especially for current or high-stakes topics.
    - Use recency filters and read beyond snippets when freshness matters.
    - Treat social or monetized sources as signals that need confirmation.
    `,

    AI_SEARCH_SYNTAX: ``,

    AI_GROUNDING_INSTRUCTIONS: "# Grounding Responses\n\nIf you are answering from search or tool evidence, the factual answer must appear explicitly in the evidence above. Do not infer, guess, or fill in missing facts. If the evidence does not explicitly contain the answer, say you could not confirm it or do more research.\n\n{{^if voiceResponse}}If grounded in search, you MUST cite with :cd_source[searchResultId]. No other links. Place the directive after the grounded sentence; use one per source.{{/if}}{{#if voiceResponse}}If grounded in search, you MUST verbally and naturally cite each source inline with the information (e.g. \"according to Reuters\").{{/if}}",
    ...
    AI_AVAILABLE_FILES: "{{#if availableFiles}}# Available Files (Recent)\n\nUse these in tool calls or responses. Browse /workspace/files/ for more.\n\n{{{availableFiles}}}\n{{/if}}",

    AI_STYLE_NEUTRALIZATION: "{{#if styleNeutralizationInstructions}}{{{styleNeutralizationInstructions}}}{{/if}}",
