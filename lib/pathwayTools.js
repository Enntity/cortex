...
const callTool = async (toolName, args, toolDefinitions, pathwayResolver) => {
    ...
    try {
        ...
        toolResult = await callPathway(pathwayName, mergedArgs, pathwayResolver);
 ...
 * @param {object} toolParams - All tool parameters (will be sanitized for large objects)
 */
const sendToolStart = async (requestId, toolCallId, toolIcon, userMessage, toolName, toolParams = null) => {
    try {
        // Sanitize tool parameters to avoid sending huge objects
        let sanitizedParams = null;
        if (toolParams && typeof toolParams === 'object') {
            sanitizedParams = {};
            for (const [key, value] of Object.entries(toolParams)) {
                // Skip very large arrays/objects (like chatHistory)
                if (key === 'chatHistory' && Array.isArray(value)) {
                    sanitizedParams[key] = `[${value.length} messages]`;
                } else if (Array.isArray(value) && value.length > 50) {
                    sanitizedParams[key] = `[${value.length} items]`;
                } else if (typeof value === 'object' && value !== null && Object.keys(value).length > 20) {
                    sanitizedParams[key] = `[object with ${Object.keys(value).length} keys]`;
                } else {
 ...
 */
const sendToolFinish = async (requestId, toolCallId, success, error = null, toolName = null) => {
    try {
        const toolMessage = {
            type: 'finish',
            callId: toolCallId,
...
/**
 * Send an app command to the client via the info channel
 * This is the unified way to control the client app from Cortex tools.
 * 
 * @param {string} requestId - The request ID
 * @param {object} command - The app command object
 * @param {string} command.type - Command type (e.g., 'showOverlay', 'createEntity', 'navigate', 'toast')
 * @param {...any} command.* - Additional fields specific to the command type
 * 
 * @example
 * // Show overlay with playlist
 * await sendAppCommand(requestId, {
 *   type: 'showOverlay',
 *   items: [
 *     { type: 'image', url: 'https://...', duration: 5, label: 'First image' },
 *     { type: 'text', content: 'Check this out!', duration: 8 },
 *     { type: 'video', url: 'https://...', duration: 15 }
 *   ],
 *   entityId: 'entity-123'
 * });
 * 
 * @example
 * // Entity creation start
 * await sendAppCommand(requestId, {
 *   type: 'createEntity',
 *   status: 'start',
 *   name: 'Nova',
 *   avatarText: 'A friendly AI with blue eyes...',
 *   avatarIcon: '✨',
 *   identity: 'Helpful and curious'
 * });
 * 
 * @example
 ...
 * });
 */
const sendAppCommand = async (requestId, command) => {
    try {
        if (!command || typeof command !== 'object' || !command.type) {
            logger.warn('sendAppCommand called without valid command object');
            return;
        }
        ...
        logger.error(`sendAppCommand error: ${error.message}`);
    }
};
...
export { callPathway, gpt3Encode, gpt3Decode, say, callTool, addCitationsToResolver, sendToolStart, sendToolFinish, sendAppCommand, withTimeout };