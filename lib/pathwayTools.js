...
/**
 * Send avatar image/video URLs to the client via the info channel
 * @param {string} requestId - The request ID
 * @param {Array} files - Array of file objects with url, gcs, hash, filename
 * @param {object} options - Optional additional metadata
 * @param {number} options.duration - Duration in milliseconds to display each image/video
 * @param {string} options.description - Optional description
 */
const sendAvatarImage = async (requestId, files, options = {}) => {
    try {
        if (!files || !Array.isArray(files) || files.length === 0) {
            logger.warn('sendAvatarImage called without files array');
            return;
        }
        ...
        if (options.duration !== undefined && options.duration !== null) {
            avatarMessage.duration = options.duration;
        }

        if (options.description) {
            avatarMessage.description = options.description;
        }
        ...
        logger.error(`sendAvatarImage error: ${error.message}`);
    }
};

export { callPathway, gpt3Encode, gpt3Decode, say, callTool, addCitationsToResolver, sendToolStart, sendToolFinish, sendAvatarImage };