...
import { ENTITY_RUN_STAGE, ENTITY_RUN_STATUS } from './constants.js';
import { buildOrientationPacket } from './orientation.js';
import { resolveAuthorityEnvelope, resolveEntityModelPolicy, getRuntimeOrigin } from './policy.js';
...
export class EntityRuntime {
    ...
    async startRun({ entityConfig = {}, args = {}, resolver = {}, goal, requestedOutput, parentRunId = null } = {}) {
        ...
        const runtimeGoal = goal || extractGoal(args);
        ...
        const doc = {
            ...
            stageHistory: [
                { stage: ENTITY_RUN_STAGE.orient, note: 'Run created', at: new Date().toISOString(), meta: { origin } },
                { stage: ENTITY_RUN_STAGE.plan, note: 'Initial planning state established', at: new Date().toISOString(), meta: {} },
            ],
        };
    ...
    async resumeRun({ runId, entityId, origin = null, trigger = 'manual', resolver = {} } = {}) {
        if (!this.store.isConfigured()) return null;
        let run = null;
        try {
            run = runId
                ? await this.store.getRun(runId)
                : await this.store.findLatestOpenRun({ entityId, origin });
        } catch (error) {
    ...
    async completeRun({ runId, result = '', stopReason = 'completed', budgetState = {}, resultData = null } = {}) {
        if (!runId || !this.store.isConfigured()) return null;
    ...
    async pauseRun({ runId, reason = 'paused', budgetState = {}, reflection = '', resultData = null } = {}) {
        if (!runId || !this.store.isConfigured()) return null;
