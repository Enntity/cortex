...
function extractFileMetadataFromContent(contentObj) {
    ...
    if (contentObj.type === 'image_url' && contentObj.image_url?.url) {
        files.push({ url: contentObj.image_url.url, type: 'image_url' });
    } else if (contentObj.type === 'file' && contentObj.url) {
        files.push({ url: contentObj.url, type: 'file' });
    } else if (contentObj.url && (contentObj.type === 'image_url' || !contentObj.type)) {
        files.push({ url: contentObj.url, type: contentObj.type || 'file' });
    }
    return files;
}
...
function buildFileCreationResponse(successfulFiles, options = {}) {
    ...
    const files = successfulFiles.map((item) => {
        if (item.fileEntry) {
            ...
            return { displayFilename: fe.displayFilename || null, url: fe.url || item.url, addedDate: fe.addedDate || null, tags: Array.isArray(fe.tags) ? fe.tags : [] };
        }
        return { displayFilename: null, url: item.url, addedDate: null, tags: [] };
    });
    ...
    const response = {
        success: true, count,
        message: `${action} complete. ${count} ${mediaType}(s) uploaded and added to file collection. ${displayInstruction} Reference files by displayFilename.`,
        files
    };
...
async function generateFileMessageContent(fileParam, agentContext) {
    if (!fileParam || typeof fileParam !== 'string') return null;
    try { if (isYoutubeUrl(fileParam)) return { type: 'image_url', url: fileParam }; } catch {}
    if (!agentContext || !Array.isArray(agentContext) || agentContext.length === 0) return null;
    ...
    return { type: 'image_url', url: resolvedUrl };
}
