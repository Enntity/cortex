 ...
 * Uses xxhash64 to match the hash format used in enntity and cortex file handler
...
async function loadFileCollection(agentContext, options = {}) {
    ...
    if (chatIds !== null) {
        ...
        // Files with inCollection === undefined/null/[] are included (Enntity uploads, not in collection - treat the same)
        // Note: false is not a valid value (normalizes to undefined), so we shouldn't see it in data
        filtered = deduped.filter(file => {
            const ic = file.inCollection;
            // Include undefined/null/empty array (Enntity uploads, not in collection - treat the same)
...
/**
 * Get the appropriate inCollection value based on chatId and entityId
 ...
 * @param {string|null|undefined} entityId - Optional entity ID (used as fallback if no chatId)
 * @returns {Array<string>} Array with chatId if provided, entityId if no chatId, otherwise ['*'] for global
 */
function getInCollectionValue(chatId = null, entityId = null) {
    if (chatId && typeof chatId === 'string' && chatId.trim() !== '') {
        return [chatId];
    }
    // Fall back to entityId if no chatId - files are global to the entity
...
/**
 * Add a chatId (or entityId as fallback) to an existing inCollection array (reference counting)
 ...
 * @param {string|null} entityId - Entity ID to use as fallback if no chatId
 * @returns {Array<string>} Updated inCollection array
 */
function addChatIdToInCollection(existingInCollection, chatId, entityId = null) {
    ...
    // Determine the ID to add: chatId if provided, otherwise entityId
    const idToAdd = (chatId && typeof chatId === 'string' && chatId.trim() !== '')
        ? chatId
        : (entityId && typeof entityId === 'string' && entityId.trim() !== '')
            ? entityId
            : null;
    
    // If no ID to add, return existing or default to global
    if (!idToAdd) {
 ...
 * @param {string|null} entityId - Optional entity ID, used as fallback for inCollection if no chatId
 * Note: Does NOT update CFH core fields (url, gcs, hash, filename) - those are managed by CFH
 * @returns {Promise<boolean>} True if successful
 */
async function updateFileMetadata(contextId, hash, metadata, contextKey = null, chatId = null, entityId = null) {
    if (!contextId || !hash) {
        return false;
    }
    ...
    try {
        ...
        const fileData = {
            ...
            // Handle inCollection: normalize if provided, otherwise preserve existing or default based on chatId/entityId
            ...
            inCollection: metadata.inCollection !== undefined
                ...
                : (existingData.inCollection !== undefined
                    ...
                    : getInCollectionValue(chatId, entityId)),
 ...
 * @param {string|null} chatId - Optional chat ID, used for inCollection value (chat-scoped if provided, entity-scoped if entityId provided, global otherwise)
 * @param {string|null} entityId - Optional entity ID, used as fallback for inCollection if no chatId
 * @returns {Promise<boolean>} True if successful
 */
async function saveFileCollection(contextId, contextKey, collection, chatId = null, entityId = null) {
    const cacheKey = getCollectionCacheKey(contextId, contextKey);
        ...
        for (const file of collection) {
            ...
            if (needsUpdate) {
                ...
                const fileData = {
                    ...
                    // Add chatId/entityId to existing inCollection (reference counting) - file may be used in multiple chats
                    inCollection: existingData.inCollection 
                        ? addChatIdToInCollection(existingData.inCollection, chatId, entityId)
                        : getInCollectionValue(chatId, entityId)
                };
 ...
 * @param {string|null} entityId - Optional entity ID, used as fallback for inCollection and filtering if no chatId
...
async function syncAndStripFilesFromChatHistory(chatHistory, agentContext, chatId = null, entityId = null) {
    ...
    // Load ALL files from all contexts (no chatIds = no filtering)
    // This finds files even if they don't have inCollection set (Enntity uploads)
    const allFiles = await loadFileCollection(agentContext);
    
    // For availableFiles output, filter to files with matching inCollection
    // Priority: chatId > entityId > all files with any inCollection
    const filterIds = chatId ? [chatId] : (entityId ? [entityId] : null);
    const collection = filterIds 
        ? await loadFileCollection(agentContext, { chatIds: filterIds })
    ...
    const updateStrippedFile = (contentObj) => {
        ...
        // Update lastAccessed and add chatId/entityId to inCollection (reference counting)
        // If this file is being used in a new chat, add that chat to the list
        // If file doesn't have inCollection set yet (e.g., uploaded by Enntity), initialize it
        const updatedInCollection = file.inCollection 
            ? addChatIdToInCollection(file.inCollection, chatId, entityId)
            : getInCollectionValue(chatId, entityId);
        updateFileMetadata(file._contextId, hash, {
            lastAccessed: now,
            inCollection: updatedInCollection
        }, fileContextKey, chatId, entityId).catch((err) => {
...
export async function resolveFileParameter(fileParam, agentContext, options = {}) {
    ...
    try {
        ...
        if (foundFile) {
            ...
            // Otherwise return the regular URL (Azure) with fresh SAS token
            if (foundFile.url) {
                // Refresh SAS for external service consumption
                if (foundFile.hash && MEDIA_API_URL) {
                    const refreshed = await ensureShortLivedUrl(foundFile, MEDIA_API_URL, foundFile._contextId || null);
                    return refreshed.url;
                }
                return foundFile.url;  // Fallback: no hash or no file handler
            }
        }
