...
async function getXXHashInstance() {
    ...
    if (xxhashInitPromise) return await xxhashInitPromise;
...
async function computeFileHash(filePath) {
    ...
    return new Promise((resolve, reject) => {
        ...
        s.on('data', (data) => xxh64.update(data));
        s.on('end', () => resolve(xxh64.digest().toString(16)));
...
function extractFileMetadataFromContent(contentObj) {
    ...
    if (contentObj.type === 'image_url' && contentObj.image_url?.url) {
        files.push({ url: contentObj.image_url.url, gcs: contentObj.gcs || null, hash: contentObj.hash || null, type: 'image_url' });
    } else if (contentObj.type === 'file' && contentObj.url) {
        files.push({ url: contentObj.url, gcs: contentObj.gcs || null, hash: contentObj.hash || null, type: 'file' });
    } else if (contentObj.url && (contentObj.type === 'image_url' || !contentObj.type)) {
        files.push({ url: contentObj.url, gcs: contentObj.gcs || null, hash: contentObj.hash || null, type: contentObj.type || 'file' });
    }
    return files;
}
...
function injectFileIntoChatHistory(chatHistory, fileContent) {
    ...
    const fileAlreadyExists = existingFiles.some(f =>
        (fileUrl && f.url === fileUrl) || (fileGcs && f.gcs === fileGcs) || (fileHash && f.hash === fileHash)
    );
...
function buildFileCreationResponse(successfulFiles, options = {}) {
    const { mediaType = 'image', action = 'Generation', legacyUrls = [] } = options;
    const files = successfulFiles.map((item) => {
        if (item.fileEntry) {
            const fe = item.fileEntry;
            return { hash: fe.hash || null, displayFilename: fe.displayFilename || null, url: fe.url || item.url, addedDate: fe.addedDate || null, tags: Array.isArray(fe.tags) ? fe.tags : [] };
        }
        return { hash: item.hash || null, displayFilename: null, url: item.url, addedDate: null, tags: [] };
    });
    ...
    if (legacyUrls && legacyUrls.length > 0) response.imageUrls = legacyUrls;
...
async function listFilesForContext(contextId, options = {}) {
    ...
    try {
        ...
        let result = files.map(f => ({
            filename: f.filename,
            displayFilename: f.filename, // file handler strips hash prefix
            hash: f.hash,
            url: f.url,
            size: f.size,
            contentType: f.contentType,
            lastModified: f.lastModified,
            name: f.name, // full GCS path
        }));
    ...
    for (const ctx of contexts) {
        ...
        for (const scope of scopes) {
            ...
            for (const f of files) {
                if (f.hash && seenHashes.has(f.hash)) continue;
...
/**
 * Find a file in a collection array by hash, filename, URL, or partial match.
 */
function findFileInCollection(fileParam, collection) {
    ...
    for (const file of collection) {
        if (file.hash === trimmed) return file;
        if (file.url === trimmed) return file;
        if (file.name === trimmed) return file;
    ...
    if (normalizedParam.length >= 4) {
        for (const file of collection) {
            ...
            if (file.name && file.name.toLowerCase().includes(normalizedParam)) return file;
        }
    }
    return null;
}
...
/**
 * Ensure a file object has a fresh short-lived (signed) URL.
 */
async function ensureShortLivedUrl(fileObject, fileHandlerUrl, contextId = null, shortLivedMinutes = 5) {
    if (!fileObject || !fileObject.hash || !fileHandlerUrl) return fileObject;
    if (fileObject.url && isYoutubeUrl(fileObject.url)) return fileObject;
    try {
        const data = await _fileHandlerRequest('get', {
        ...
        if (data && data.url) {
            const shortLivedUrl = data.shortLivedUrl || data.url;
            return { ...fileObject, url: shortLivedUrl };
        }
    } catch (err) {
        logger.warn(`Failed to resolve short-lived URL for file ${fileObject.hash}: ${err.message}`);
    }
    return fileObject;
}

/**
 * Check if a file exists by hash via file handler.
 * Returns {url, hash, filename, shortLivedUrl} or null.
 */
async function checkHashExists(hash, fileHandlerUrl, pathwayResolver = null, contextId = null, shortLivedMinutes = 5) {
    if (!hash || !fileHandlerUrl) return null;
    try {
        ...
        const response = await axios.get(url, { timeout: 10000, validateStatus: s => s >= 200 && s < 500 });
        if (response.status === 200 && response.data?.url) {
            const d = response.data;
            return { url: d.url, hash: d.hash || hash, filename: d.filename || null, shortLivedUrl: d.shortLivedUrl || null };
        }
        return null;
    } catch (err) {
        const msg = err?.message || String(err);
        if (pathwayResolver?.logWarning) pathwayResolver.logWarning(`checkHash failed: ${msg}`);
        return null;
    }
}
...
async function deleteFileByHash(hash, pathwayResolver = null, contextId = null) {
    if (!hash || typeof hash !== 'string') { logger.warn('deleteFileByHash: hash is required'); return false; }
    try {
        const url = buildFileHandlerUrl(MEDIA_API_URL, { hash, ...(contextId ? { contextId } : {}) });
        const response = await axios.delete(url, { validateStatus: s => s >= 200 && s < 500, timeout: 30000 });
        if (response.status === 200) { logger.info(`Deleted file with hash ${hash}`); return true; }
        if (response.status === 404) { logger.info(`File with hash ${hash} not found`); return false; }
        return false;
    } catch (err) {
        if (err?.response?.status === 404) return false;
        logger.warn(`Error deleting file with hash ${hash}: ${err?.message || err}`);
        return false;
    }
}
...
async function uploadFileToCloud(fileInput, mimeType = null, filename = null, pathwayResolver = null, contextId = null) {
    ...
    try {
        ...
        if (fileBuffer) {
            ...
            const existing = await checkHashExists(fileHash, fileHandlerUrl, pathwayResolver, contextId);
        ...
        if (fileHash) formData.append('hash', fileHash);
        ...
        if (uploadResponse.data?.url) {
            ...
            return { url: d.url, hash: d.hash || fileHash, shortLivedUrl: d.shortLivedUrl || null };
        }
...
const uploadImageToCloud = async (imageInput, mimeType, filename, pathwayResolver = null, contextId = null) => {
    return await uploadFileToCloud(imageInput, mimeType, filename, pathwayResolver, contextId);
};

/**
 * Add a file to the collection. Ensures the file is in GCS (uploads if URL provided).
 * No Redis writes — GCS is the source of truth.
 */
async function addFileToCollection(contextId, contextKey, url, gcs, filename, tags = [], notes = '', hash = null, fileUrl = null, pathwayResolver = null, permanent = false, chatId = null, entityId = null) {
    if (!contextId || !filename) throw new Error("contextId and filename are required");
    if (fileUrl && isYoutubeUrl(fileUrl)) throw new Error("YouTube URLs cannot be added to the file collection.");
    if (url && isYoutubeUrl(url)) throw new Error("YouTube URLs cannot be added to the file collection.");

    let finalUrl = url;
    let finalHash = hash;

    if (fileUrl && (!url || !url.startsWith('gs://'))) {
    ...
    if (!finalUrl) throw new Error("url or fileUrl is required");

    const mimeType = determineMimeTypeFromUrl(finalUrl, gcs, null);
...
export async function resolveFileParameter(fileParam, agentContext, options = {}) {
    ...
    try {
        ...
        if (foundFile) {
            if (foundFile.hash && MEDIA_API_URL) {
                const defaultCtx = getDefaultContext(agentContext);
                const refreshed = await ensureShortLivedUrl(foundFile, MEDIA_API_URL, defaultCtx?.contextId || null);
...
async function generateFileMessageContent(fileParam, agentContext) {
    if (!fileParam || typeof fileParam !== 'string') return null;
    try { if (isYoutubeUrl(fileParam)) return { type: 'image_url', url: fileParam, gcs: null, hash: null }; } catch {}
    if (!agentContext || !Array.isArray(agentContext) || agentContext.length === 0) return null;
    ...
    const defaultCtx = getDefaultContext(agentContext);
    const resolved = await ensureShortLivedUrl(foundFile, MEDIA_API_URL, defaultCtx?.contextId || null);
    return { type: 'image_url', url: resolved.url, gcs: null, hash: resolved.hash || null };
}
...
async function syncAndStripFilesFromChatHistory(chatHistory, agentContext, chatId = null, entityId = null) {
    ...
    const isInCollection = (contentObj) => {
        ...
        if (fileHash && collectionByHash.has(fileHash)) return true;
...
/**
 * Resolve file hashes to content format suitable for LLM processing.
 */
async function resolveFileHashesToContent(fileHashes, cfg, contextId = null) {
    if (!fileHashes || fileHashes.length === 0) return [];
    return Promise.all(fileHashes.map(async (hash) => {
        try {
            const fileHandlerUrl = cfg?.get?.('whisperMediaApiUrl');
            if (fileHandlerUrl && fileHandlerUrl !== 'null') {
                const url = buildFileHandlerUrl(fileHandlerUrl, { hash, checkHash: true, shortLivedMinutes: 5, ...(contextId ? { contextId } : {}) });
                const response = await axios.get(url, { timeout: 10000, validateStatus: s => s >= 200 && s < 500 });
                if (response.status === 200 && response.data?.url) {
                    const d = response.data;
                    const shortLivedUrl = d.shortLivedUrl || d.url;
                    return JSON.stringify({ type: "image_url", url: shortLivedUrl, image_url: { url: shortLivedUrl }, hash });
                }
            }
            return JSON.stringify({ type: "file_hash", hash, _cortex_needs_resolution: true });
        } catch (error) {
            return JSON.stringify({ type: "file_error", hash, error: error.message });
        }
    }));
}
...
// ─── Deprecated / No-op stubs (kept for backward compatibility) ──────────────
