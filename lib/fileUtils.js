...
/**
 * Get a signed (short-lived) HTTPS URL for a gs:// URL.
 * Calls the file handler's signUrl endpoint.
 * @param {string} gcsUrl - gs://bucket/path URL
 * @param {number} minutes - URL expiration in minutes (default 5)
 * @returns {Promise<string|null>} Signed HTTPS URL, or null on failure
 */
async function getSignedFileUrl(gcsUrl, minutes = 5) {
    if (!gcsUrl || !gcsUrl.startsWith('gs://')) return null;
    try {
        ...
        return data?.shortLivedUrl || null;
    } catch (err) {
        logger.warn(`Failed to get signed URL for ${gcsUrl}: ${err.message}`);
        return null;
    }
}
...
export async function resolveFileParameter(fileParam, agentContext, options = {}) {
    ...
    try {
        ...
        if (foundFile) {
            // listFolder results already include signed URLs;
            // if the URL is a gs:// URL, get a signed version
            if (foundFile.url && foundFile.url.startsWith('gs://')) {
                const signedUrl = await getSignedFileUrl(foundFile.url);
                return signedUrl || foundFile.url;
            }
...
async function generateFileMessageContent(fileParam, agentContext) {
    ...
    // If URL is gs://, get a signed version; otherwise use as-is (already signed from listFolder)
    ...
    return { type: 'image_url', url: resolvedUrl, gcs: null, hash: foundFile.hash || null };
}
 ...
 * Uses listFilesForContext to find files, then returns signed URLs.
 */
async function resolveFileHashesToContent(fileHashes, cfg, contextId = null) {
    if (!fileHashes || fileHashes.length === 0) return [];

    // Load all files for this context to look up hashes
    let collection = [];
    if (contextId) {
        try {
            collection = await listFilesForContext(contextId);
        } catch (err) {
            logger.warn(`resolveFileHashesToContent: failed to list files for ${contextId}: ${err.message}`);
        }
    }

    return Promise.all(fileHashes.map(async (hash) => {
        try {
            // Find the file by hash in the collection
            const file = collection.find(f => f.hash === hash);
            if (file) {
                // listFolder results already include signed URLs
                ...
                return JSON.stringify({ type: "image_url", url: resolvedUrl, image_url: { url: resolvedUrl }, hash });
            }
