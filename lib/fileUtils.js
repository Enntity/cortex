 ...
 * @param {number} minutes - URL expiration in minutes (default 60)
 * @returns {Promise<string|null>} Signed HTTPS URL, or null on failure
 */
async function getSignedFileUrl(fileRef, minutes = 60) {
...
export async function resolveFileParameter(fileParam, agentContext, options = {}) {
    ...
    try {
        ...
        if (foundFile) {
            const signedUrl = foundFile.blobPath
                ? await getSignedFileUrl(foundFile.blobPath, options.minutes || 60)
                : null;
            return signedUrl || foundFile.url || null;
        }
        return null;
    } catch (error) {
...
async function generateFileMessageContent(fileParam, agentContext) {
    ...
    const signedUrl = foundFile.blobPath
        ? await getSignedFileUrl(foundFile.blobPath, 60)
        : null;
    const resolvedUrl = signedUrl || foundFile.url;
...
function formatFilesForTemplate(collection) {
    ...
    const fileList = recentFiles.map((file) => {
        ...
        const stableReference = file.blobPath || file.url || '';
        ...
        return `${displayFilename} | ${stableReference} | ${dateAdded}${tags ? ' | ' + tags : ''}`;
    }).join('\n');
    let result = fileList;
