...
class PathwayManager {
  ...
  _createPromptObject(promptItem, systemPrompt, defaultName = null) {
    ...
    // Store file references for later resolution.
    if (promptFiles.length > 0) {
      prompt.fileRefs = promptFiles;
    }
  ...
  async transformPrompts(pathway) {
    ...
    // Collect all file references from all prompts.
    const allFileRefs = newPathway.prompt
      .filter(p => p.fileRefs && p.fileRefs.length > 0)
      .flatMap(p => p.fileRefs);

    // Store file references at pathway level for later resolution.
    if (allFileRefs.length > 0) {
      newPathway.fileRefs = [...new Set(allFileRefs)];
    }
