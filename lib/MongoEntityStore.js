 ...
 *   - image: Object (optional - { url, blobPath, filename })
 *   - video: Object (optional - { url, blobPath, filename })
 * - tools: [String] (tool names - explicit list preferred, ["*"] for all supported for backward compat)
 * - resources: [{ url, blobPath, filename, type }] (attached media/documents)
 ...
 * - modelPolicy: Object (optional - curated model profile / stage policy)
 * - authorityProfile: Object (optional - per-run budget envelope defaults)
 * - autonomyProfile: Object (optional - runtime/autonomy preferences)
 * - defaultChildPolicy: Object (optional - child-run defaults)
 * - createdAt: Date
 * - updatedAt: Date
 */
...
export class MongoEntityStore {
    ...
    async getEntity(entityId, options = {}) {
        ...
        // Return from cache if: not fresh requested, entity exists, and not stale.
        // Individual cache entries can be authoritative even when the full-cache
        // bootstrap has not run yet (for example after direct upserts or tests
        // that seed a specific entity into the store).
     ...
     * @param {Object} [entity.modelPolicy] - Curated model profile / stage policy
     * @param {Object} [entity.authorityProfile] - Default authority envelope
     * @param {Object} [entity.autonomyProfile] - Runtime/autonomy preferences
     * @param {Object} [entity.defaultChildPolicy] - Default child-run policy
    ...
    async upsertEntity(entity) {
        ...
        try {
            ...
            const doc = {
                ...
                avatar: entity.avatar || null, // Optional: { text, image: {url, blobPath, filename}, video: {url, blobPath, filename} }
                ...
                baseModel: entity.baseModel || null, // Optional: legacy base model metadata
                reasoningEffort: entity.reasoningEffort || null, // Optional: reasoning effort level
                modelPolicy: entity.modelPolicy || null, // Optional: curated model profile / stage policy
                authorityProfile: entity.authorityProfile || null, // Optional: runtime authority envelope defaults
                autonomyProfile: entity.autonomyProfile || null, // Optional: autonomy preferences
                defaultChildPolicy: entity.defaultChildPolicy || null, // Optional: child-run defaults
                ...
                secrets: entity.secrets || null, // Optional: encrypted key-value secrets { KEY: "iv:tag:encrypted", ... }
                updatedAt: now
