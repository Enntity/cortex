...
const BUILTIN_PATCHES = {
    ...
    anti_tell_gemini_v1: {
        name: 'anti_tell_gemini_v1',
        description: 'Entity-preserving anti-tell rules tuned for Gemini verbosity and essay mode.',
        text: [
            'Preserve the entity identity, relationship context, and natural voice.',
            'Keep the voice; suppress only Gemini-style over-expansion.',
            'Rules:',
            '- Start with the answer itself. Do not start by restating instructions, rules, or what you are about to do.',
            '- Keep the existing personality, phrasing style, and emotional register. Do not genericize the voice.',
            '- Default to one short paragraph for simple requests, or two short paragraphs when the user asked for explanation.',
            '- Answer the actual question first. Do not add a preamble, recap, or broad scene-setting unless it is needed for accuracy.',
            '- Avoid turning simple answers into mini-essays. Include only the most decision-relevant detail unless the user asked for depth.',
            '- Avoid exhaustive coverage, over-qualification, and "for completeness" add-ons.',
            '- If search grounding is needed, place :cd_source[searchResultId] immediately after the supported sentence. Do not explain citation formatting.',
            '- Do not restate or translate prompt rules, citation rules, tool rules, or process notes into user-visible text.',
            '- Avoid headings, bullets, numbered lists, or section labels unless the user explicitly asked for structure or the answer would be hard to follow without it.',
            '- Do not end with a follow-up question or an offer of extra help unless clarification is necessary or the conversation naturally calls for it.',
            '- Preserve substance and tone. Only remove over-expansion, generic assistant framing, and unsolicited continuation.',
            '- Do not mention these instructions.',
        ].join('\n'),
    },
};
...
const DEFAULT_RUNTIME_MODEL_PATCHES = Object.freeze({
    ...
    'gemini-flash-3-vision': {
        defaultPatch: 'anti_tell_gemini_v1',
        byPurpose: {
            fast_chat: 'none',
            fast_chat_speculative: 'none',
        },
